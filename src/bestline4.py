...
class Player(BasePlayer):
    ...
    def init_turn(self, board, nodes, max_units):
        ...
        self.friendly_degree = {}
        for node in self.frontier:
            self.friendly_degree[node] = self.get_friendly_degree(node)
        """
    ...
    def get_friendly_degree(self, node):
        deg = 0
        for neighbor in self.board[node]:
            if self.board.nodes[neighbor]['owner'] == self.player_num:
                deg += 1
    ...
    def player_place_units(self):
        ...
        while len(assignments) > 0:
            assignments = sorted(
                    assignments,
                    key=lambda x: (x[2] - remaining_units[x[0]] + \
                            (5 if self.board.nodes[x[1]]['owner'] is None else 0)) - \
                            self.friendly_degree[x[1]]
            )
